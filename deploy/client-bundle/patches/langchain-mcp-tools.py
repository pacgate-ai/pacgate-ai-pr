...
def convert_mcp_tool_to_langchain_tool(
    ...
    *,
    ...
    if tool_name_prefix and server_name:
        # Pacgate: normalize hyphens to underscores in the server-name prefix so
        # LLMs can transcribe the tool name correctly. MCP server keys may use
        # hyphens (e.g. "yuandian-law"), which produces tool names like
        # "yuandian-law_yuandian_rh_ft_search" that models hallucinate as
        # "yuandian_law_yuandian_rh_ft_search". Function names must be
        # underscore-separated for reliable function-calling.
        safe_server_name = server_name.replace("-", "_")
        lc_tool_name = f"{safe_server_name}_{tool.name}"
