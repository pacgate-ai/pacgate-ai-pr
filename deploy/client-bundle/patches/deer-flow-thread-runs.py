...
class RunCreateRequest(BaseModel):
    ...
    # Pacgate: default to "interrupt" so a new message during a long agent run
    # cancels the stale run instead of rejecting the request with 409 Conflict.
    # The frontend never sends multitask_strategy, so the default governs all
    # chat submissions.
    multitask_strategy: Literal["reject", "rollback", "interrupt", "enqueue"] = Field(
        default="interrupt", description="Concurrency strategy"
    )
